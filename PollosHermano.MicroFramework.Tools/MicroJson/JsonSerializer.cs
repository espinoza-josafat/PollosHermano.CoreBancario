...
namespace PollosHermano.MicroFramework.Tools.MicroJson
{
    ...
    public class JsonSerializer
    {
        ...
        object Deserialize(object from, Type type)
        {
            if (from == null)
                return null;
        ...
        void DeserializeArray(IList from, Array to, Type itemType)
        {
        ...
        void DeserializeList(IList from, IList to)
        {
            var itemType = to.GetType().GetProperty("Item").PropertyType;
        ...
        void DeserializeDictionary(IEnumerable<KeyValuePair<string, object>> from, object to)
        {
            var type = to.GetType();
        ...
        Dictionary<string, SetterMember> MemberCache = new Dictionary<string, SetterMember>();

        SetterMember GetMember(Type type, string name)
        {
            SetterMember member;
        ...
        static string SerializeDateTime(object o)
        {
            var d = (DateTime)o;
        ...
        string SerializeComplexType(object o)
        {
        ...
        void SerializeProperties(object o, StringBuilder s)
        {
            var type = o.GetType();
        ...
        void SerializeDictionary(object o, StringBuilder s)
        {
        ...
        Dictionary<Type, GetterMember[]> MembersCache = new Dictionary<Type, GetterMember[]>();

        GetterMember[] GetMembers(Type type)
        {
            GetterMember[] members;

            if (!MembersCache.TryGetValue(type, out members))
            {
                var ifAnonymousType = CheckIfAnonymousType(type);

                var props = (IEnumerable<GetterMember>)null;

                if (ifAnonymousType)
                    props = type.GetProperties()
                        .Select(p => BuildGetterMember(p));
                else
                    props = type.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase)
        ...
        static GetterMember BuildGetterMember(PropertyInfo p)
        {
        ...
        static GetterMember BuildGetterMember(FieldInfo f)
        {
